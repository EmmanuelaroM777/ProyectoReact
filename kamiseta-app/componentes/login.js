...
export default function Login({ onLoginSuccess, onIrRegistro }) {
  ...
  const handleLogin = async () => {
    try {
      const res = await axios.post('http://localhost:3001/api/usuarios/login', { email, clave });
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        <View style={styles.overlay}>
          ...
          <TouchableOpacity
            style={[styles.boton, { backgroundColor: '#28a745', marginTop: 15 }]}
...
const styles = StyleSheet.create({
  ...
  overlay: {
    ...
    justifyContent: 'center',
    padding: 20,
    backgroundColor: 'rgba(0,0,0,0.4)' // oscurece para mejor contraste
  },
  titulo: {
    fontSize: 24,
    textAlign: 'center',
